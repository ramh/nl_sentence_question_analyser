...
#RULES = { "ROOT-S-VP-VB" => "what",
#          "ROOT-S-VP-PP-NP-NN" => "where",
#          "ROOT-S-VP-NP-PRP" => "who" }
...
def process_node(stanford_parser_tree_string, root_node)
  ...
  if stanford_parser_tree_string =~ /^\([^\)]+\(/
    ...
    child = Tree::TreeNode.new(node_name[1], "NON-LEAF")   #(root_node.content.to_i + 1).to_s)
    root_node << child
    ...
    if sub_strings.length == 1
      ...
      child = Tree::TreeNode.new(node_name[1], node_content[1])
      root_node << child
    else
